...
def write_gro_file(filename, cell_grid, lattice, mask, morph_type):
    ...
    -----------
        ...
        mask: numpy.ndarray
            A mask grid where elements inside the shape are 1 and outside are 0.
        morph_type: str
            The name of the glycine polymorph ('alpha', 'beta' or 'gamma').
    """
...
def collect_beta_molecules(cell_grid, mask):
    """
    Takes the random ordering of the coordinates in the beta glycine unit cell and groups atoms by molecules.
    A secondary result of this operation is that stray (incomplete) molecules are removed.
        ...
        for j in range(ry):
            for k in range(rz):
                ...
                if mask[i][j][k] == 1:
                    ...
                    #Check if the first almost complete molecule can be completed (if the x+ neighbor cells are present)
                    if padded_mask[i+2][j+1][k+1]:
                        
                        #Add coordinates such that they correspond to Gly molecule
                        cell_coordinates.append(cell_grid[i+1][j][k][0][8]) # N from x+ neighbor
                        cell_coordinates.append(cell_grid[i+1][j][k][0][2]) # CA from x+ neighbor
                        cell_coordinates.append(cell_grid[i+1][j][k][0][0]) # C from x+ neighbor
                        cell_coordinates.append(cell_grid[i+1][j][k][0][18])  # O from x+ neighbor
                        cell_coordinates.append(cell_grid[i][j][k][0][16])  # O1 from own cell
                        cell_coordinates.append(cell_grid[i+1][j][k][0][14]) # H1 from x+ neighbor
                        cell_coordinates.append(cell_grid[i+1][j][k][0][10]) # H2 from x+ neighbor
                        cell_coordinates.append(cell_grid[i+1][j][k][0][6]) # HA1 from x+ neighbor
                        cell_coordinates.append(cell_grid[i][j][k][0][4]) # HA2 from own cell
                        cell_coordinates.append(cell_grid[i+1][j][k][0][12]) # H3 from x+ neighbor

                        #Add atoms such that they correspond to Gly molecule
                        cell_atoms.append(cell_grid[i+1][j][k][1][8]) # N from x+ neighbor
                        cell_atoms.append(cell_grid[i+1][j][k][1][2]) # CA from x+ neighbor
                        cell_atoms.append(cell_grid[i+1][j][k][1][0]) # C from x+ neighbor
                        cell_atoms.append(cell_grid[i][j][k][1][18])  # O from x+ neighbor
                        cell_atoms.append(cell_grid[i+1][j][k][1][16])  # O1 from own cell
                        cell_atoms.append(cell_grid[i+1][j][k][1][14]) # H1 from x+ neighbor
                        cell_atoms.append(cell_grid[i+1][j][k][1][10]) # H2 from x+ neighbor
                        cell_atoms.append(cell_grid[i+1][j][k][1][6]) # HA1 from x+ neighbor
                        cell_atoms.append(cell_grid[i][j][k][1][4]) # HA2 from own cell
                        cell_atoms.append(cell_grid[i+1][j][k][1][12]) # H3 from x+ neighbor

                    # Check if the second almost complete molecule can be completed (if the x+ and y+ neighbor cells are present)
                    if padded_mask[i+2][j+1][k+1] and padded_mask[i+1][j+2][k+1]:

                        #Add coordinates such that they correspond to Gly molecule
                        cell_coordinates.append(cell_grid[i][j][k][0][9]) # N from own cell
                        cell_coordinates.append(cell_grid[i][j][k][0][3]) # CA from own cell
                        cell_coordinates.append(cell_grid[i][j][k][0][1])  # C from own cell 
                        cell_coordinates.append(cell_grid[i][j][k][0][19])  # O from own cell
                        cell_coordinates.append(cell_grid[i+1][j][k][0][17]) # O1 from x+ neighbor
                        cell_coordinates.append(cell_grid[i][j][k][0][15])    # H1 from own cell
                        cell_coordinates.append(cell_grid[i][j][k][0][11])    # H2 from own cell
                        cell_coordinates.append(cell_grid[i][j+1][k][0][7])  # HA1 from y+ neighbor 
                        cell_coordinates.append(cell_grid[i+1][j][k][0][5])   # HA2 from x+ neighbor
                        cell_coordinates.append(cell_grid[i][j][k][0][13])   # H3 from own cell
                        
                        #Add atoms such that they correspond to Gly molecule
                        cell_atoms.append(cell_grid[i][j][k][1][9]) # N from own cell
                        cell_atoms.append(cell_grid[i][j][k][1][3]) # CA from own cell
                        cell_atoms.append(cell_grid[i][j][k][1][1])  # C from own cell 
                        cell_atoms.append(cell_grid[i][j][k][1][19])  # O from own cell
                        cell_atoms.append(cell_grid[i+1][j][k][1][17]) # O1 from x+ neighbor
                        cell_atoms.append(cell_grid[i][j][k][1][15])    # H1 from own cell
                        cell_atoms.append(cell_grid[i][j][k][1][11])    # H2 from own cell
                        cell_atoms.append(cell_grid[i][j+1][k][1][7])  # HA1 from y+ neighbor 
                        cell_atoms.append(cell_grid[i+1][j][k][1][5])   # HA2 from x+ neighbor
                        cell_atoms.append(cell_grid[i][j][k][1][13])   # H3 from own cell
...
def build_crystal_system(rx,ry,rz, morph_type, box_size, max_sol=np.inf):
    ...
    if morph_type == "beta":
        #Collect atoms by molecule and lose stray atoms for beta glycine crystal
        cell_grid = collect_beta_molecules(cell_grid, mask)
    
    if morph_type == "gamma":
        #Collect atoms by molecule and lose stray atoms for gamma glycine crystal
